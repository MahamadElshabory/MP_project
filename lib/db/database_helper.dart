...
class DatabaseHelper {
  ...
  Future _upgradeDB(Database db, int oldV, int newV) async {
    if (oldV < 2) {
      await db.execute(
          'ALTER TABLE events ADD COLUMN isRSVP INTEGER NOT NULL DEFAULT 0'
      );
    }
    if (oldV < 3) {
      await db.execute('''
  ...
  Future<ClassModel?> getClass(int id) async {
    ...
    if (maps.isNotEmpty) return ClassModel.fromMap(maps.first);
    return null;
  }
  Future<List<ClassModel>> getAllClasses() async {
    final db = await database;
    final result = await db.query('classes');
    return result.map((m) => ClassModel.fromMap(m)).toList();
  }
  Future<int> updateClass(ClassModel c) async {
    final db = await database;
    return db.update('classes', c.toMap(),
  ...
  Future<int> updateEvent(EventModel e) async {
    final db = await database;
    return db.update('events', e.toMap(),
  ...
  Future<int> updateNote(NoteModel n) async {
    final db = await database;
    return db.update('notes', n.toMap(),
  ...
  Future<int> updateProfile(ProfileModel p) async {
    final db = await database;
    return db.update('profile', p.toMap(),
